...
namespace TNCKissInterface
{
    public class Crc
    {
        ...
        static Int32 CRCSTART = 0xffff;
        static Int32 CRCFINISH = 0xf0b8;

        static Int32[] crcTable =
        {
        ...
        public static Byte[] CreateCrc(Byte[] buf)
        {
            ...
            Int32 crc = GetCrc(buf) ^ CRCSTART;
            Byte[] ret = { (Byte)(crc & 0xff), (Byte)((crc >> 8) & 0xff) };
            return ret;
        }

        public static Int32 UpdCrc(Byte b, Int32 crc)
        {
            Int32 retCRC = (((crc >> 8) & 0xff) ^ crcTable[(crc & 0xff) ^ Convert.ToInt32(b)]) & 0xFFFF;
            return retCRC;
        }

        private static Int32 GetCrc(Byte[] buf)
        {
            //
            // Compute the CRC
            //
            ...
            {
                crc = ((crc >> 8) & 0xff) ^ crcTable[(crc & 0xff) ^ Convert.ToInt32(b)];
            }
            return crc;
        }
    }
    ...
    public class WinlinkAuth
    {
        ...
        //
        static Byte[] salt = {77,197,101,206,190,249,93,200,51,243,93,237,71,94,239,138,
        ...
        {
            Byte[] retBuf = MD5ByteHash(text);
            return Encoding.ASCII.GetString(retBuf);
        }

        public static UInt32 ChallengedPassword(String challengePhrase, String password)
        {
            return ChallengedPassword(challengePhrase, password, salt);
        }

        public static UInt32 ChallengedPassword(String challengePhrase, String password, String secret)
        {
            return ChallengedPassword(challengePhrase, password, Encoding.ASCII.GetBytes(secret));
        }

        static UInt32 ChallengedPassword(String challengePhrase, String password, Byte [] slt)
        {
            //
            ...
            //
            Int32 i;
            Int32 m = challengePhrase.Length + password.Length;
            Byte [] tmpCP = new Byte[m + slt.Length]; 
            Encoding.ASCII.GetBytes(challengePhrase + password).CopyTo(tmpCP, 0);
            slt.CopyTo(tmpCP, m);
            
            Byte [] retHash = MD5ByteHash(tmpCP);
            for (i = 0; i < tmpCP.Length; i++)
            {
                //
            ...
            //
            UInt32 retVal = Convert.ToUInt32(retHash[3] & 0x3f);    // Trim the sign bit from what will be the high byte
            for (i = 2; i >= 0; i--)
            {
        ...
        static Byte [] MD5ByteHash(String text)
        {
            return MD5ByteHash(Encoding.ASCII.GetBytes(text));
        }

        static Byte[] MD5ByteHash(Byte [] iBuf)
        {
            //
            // Calculate the MD5 hash of the supplied buffer
            //
            MD5 hash = MD5.Create();
            Byte[] retBuf = hash.ComputeHash(iBuf);
            hash.Clear();
            return retBuf;
        }
    }
